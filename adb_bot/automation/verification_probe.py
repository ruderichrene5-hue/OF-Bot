...
def instagram_installed(target: str, adb_client, logger=None) -> bool:
    ...
    bad minute.
    """
    # An **empty** answer is not an answer. `Blank (21)` was written off as
    # having no Instagram on 2026-08-16 seconds after `glogin success`: the
    # shell was up but the package manager was not yet, so `pm` returned "" and
    # the phone was shut back down. "Not installed" is a verdict about a phone
    # nobody will re-check, so it has to be said only when `pm` actually
    # answered and Instagram was not in the answer.
    for attempt in range(3):
        ...
        if INSTAGRAM_PACKAGE in installed:
            return True
        if installed.strip():
            break                       # pm answered, and Instagram was not in it
        if logger is not None:
            logger.info("pm returned nothing on %s (%d/3); it may still be "
                        "starting", target, attempt + 1)
        time.sleep(4)
